...
REPO=/var/lib/jenkins/workspace/iDempiere8.2Daily
cd /tmp
rm -f Adempiere.dmp
# unzip -u ${REPO}/org.adempiere.server-feature/data/seed/Adempiere.jar
unzip -u /home/idempiere/seed82/Adempiere.jar
...
TODAY=`date +%Y%m%d`
echo "update ad_system set lastmigrationscriptapplied = '${TODAY}_Daily8.2Build' || $BUILD_NUMBER || '/' || lastmigrationscriptapplied;" | sqlplus $ADEMPIERE_DB_USER/$ADEMPIERE_DB_PASSWORD@$HOST/$DATABASE
...
mkdir -p /tmp/seed82
rm -f /tmp/seed82/Adempiere.jar
cp -p ${REPO}/org.adempiere.server-feature/data/seed/Adempiere.jar /tmp/seed82/
