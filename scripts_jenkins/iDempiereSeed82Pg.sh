...
unzip -u /home/idempiere/seed82/Adempiere_pg.jar
...
TODAY=`date +%Y%m%d`
echo "update ad_system set lastmigrationscriptapplied = '${TODAY}_Daily8.2Build' || $BUILD_NUMBER || '/' || lastmigrationscriptapplied" | $PSQLCMD
...
rm Adempiere_pg.dmp
mkdir -p /tmp/seed82
rm -f /tmp/seed82/Adempiere_pg.jar
cp -p ${REPO}/org.adempiere.server-feature/data/seed/Adempiere_pg.jar /tmp/seed82/
