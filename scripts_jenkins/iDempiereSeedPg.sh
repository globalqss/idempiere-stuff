...
TODAY=`date +%Y%m%d`
echo "update ad_system set lastmigrationscriptapplied = '${TODAY}_DailyBuild' || $BUILD_NUMBER || '/' || lastmigrationscriptapplied" | $PSQLCMD
...
rm Adempiere_pg.dmp
mkdir -p /tmp/seed
rm -f /tmp/seed/Adempiere_pg.jar
cp -p ${REPO}/org.adempiere.server-feature/data/seed/Adempiere_pg.jar /tmp/seed/
