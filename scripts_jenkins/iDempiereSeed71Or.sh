...
REPO=/var/lib/jenkins/workspace/iDempiere7.1Daily
...
TODAY=`date +%Y%m%d`
echo "update ad_system set lastmigrationscriptapplied = '${TODAY}_Daily7.1Build' || $BUILD_NUMBER || '/' || lastmigrationscriptapplied;" | sqlplus $USER/$PASS@$HOST/$DATABASE
...
mkdir -p /tmp/seed71
rm -f /tmp/seed71/Adempiere.jar
cp -p ${REPO}/org.adempiere.server-feature/data/seed/Adempiere.jar /tmp/seed71/
