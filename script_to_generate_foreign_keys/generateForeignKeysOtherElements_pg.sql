...
' ADD CONSTRAINT '||SUBSTR (REPLACE (SUBSTR (columnname, 1, LENGTH (columnname) - 5), '_', '') || '_' || REPLACE (tablename, '_', ''), 1, 30)||
...
and t.isview = 'N' and t.tablename not like 'T|_%' escape '|' and t.tablename not in ('Test')
     AND NOT EXISTS (
	SELECT 1
