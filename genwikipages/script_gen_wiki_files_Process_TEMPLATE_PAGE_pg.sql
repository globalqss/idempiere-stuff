-- run as
-- psql -d idempiere -U adempiere -q -P tuples_only=on -P footer=off -Pborder=0 -P format=unaligned -f script_gen_wiki_files_Process_TEMPLATE_PAGE_pg.sql > /tmp/script_gen_wiki_files_Process_TEMPLATE_PAGE_pg.sh
-- and then execute the generated script
SELECT
'cat > "/tmp/wiki/Template:'||translate(f.name,' /','_-')||'_(Process_ID-'||f.ad_process_id||'_V1.0.0).wiki" <<!
== Process: '||f.name||' ==
...
[[Image:'||translate(f.name,' /','_-')||'_-_Process_(iDempiere_1.0.0).png|border]]'
